...
START_TEST(print_list)
	{
		if (!stream)
			ck_abort_msg("Error allocating memory for stream");
		lrsll_list *tmp = lrsll_createList();
		lrsll_push(tmp, "B");
		lrsll_push(tmp, "A");
		lrsll_printList(tmp, stream);
		fclose(stream);
		lrsll_freeList(tmp);
		ck_assert_str_eq(streamData, "\nCurrent list - \n\tA\tB\n");
	}
END_TEST

START_TEST(print_empty_list)
	{
		if (!stream)
			ck_abort_msg("Error allocating memory for stream");
		lrsll_list *tmp = lrsll_createList();
...
Suite *list_suite(void)
{
	...
	TCase *tc_print = tcase_create("Printing");
	tcase_add_checked_fixture(tc_print, streamSetup, streamTeardown);
	tcase_add_test(tc_print, print_list);
	tcase_add_test(tc_print, print_empty_list);
...
//    Uncomment if you wish to debug unit tests or run valgrind on this file to test for leaks (valgrind --leak-check=full ./testexec)
